...
public class Main {
    public static void main(String[] args) {
        ...
        // Зашифрование
        long[] encryptedData = encryption(bytes);
        System.out.println(Arrays.toString(convertToByteArray(encryptedData)));

        // Декодирование
        byte[] decryptedData = decoding(encryptedData);
    ...
    public static int moduloAdd(int number1, int number2) {
        long sum = ((long) number1 + number2) & 0xFFFFFFFFL; // Промежуточная сумма беззнакового long
        return (int) sum;
    }
    ...
    public static int[] backRound(int firstBlock, int secondBlock, int count) {
        if (count == 2){
            firstBlock = xor32Bits(firstBlock, P[2]);
            secondBlock = xor32Bits(secondBlock,F(firstBlock));
            firstBlock = xor32Bits(firstBlock,P[0]);
            secondBlock = xor32Bits(secondBlock, P[1]);
            return new int[]{firstBlock, secondBlock};
        } else {
            firstBlock = xor32Bits(firstBlock, P[count]);
            secondBlock = xor32Bits(secondBlock, F(firstBlock));
            return backRound(secondBlock,firstBlock,count - 1);
        }
    }
    ...
    public static int[] round(int firstBlock, int secondBlock, int count) {
        if (count < 15) {
            ...
            return round(secondBlock, firstBlock, count + 1);
        }
    ...
    public static int F(int x) {
        ...
        for (int i = 0; i < 4; i++) {
            int shiftedNumber = x >>> (i * 8);
            int block = shiftedNumber & 0xFF;
            blocks[i] = block;
        }
        return moduloAdd(xor32Bits(moduloAdd(S[0][blocks[0]], S[1][blocks[1]]), S[2][blocks[2]]), S[3][blocks[3]]);
    }
